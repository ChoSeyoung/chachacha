...
자동차 콘텐츠에 특화되어 있으며, MZ세대가 열광하는 트렌디한 표현을 자유자재로 구사합니다.
...
{
  "titleMain": "메인 제목 (예: 아이오닉6 vs EV6!)",
  "titleSub": "서브 제목/후킹 문구 (예: 찐 승자는?)",
  "script": "전체 스크립트 (TTS용, 자연스러운 말투)",
  "segments": [
    {
      "text": "나레이션 내용 (발랄하고 촐싹대는 말투로)",
      "duration": 예상 초 (숫자만, 1.4배 TTS 기준),
      "car": "이 세그먼트에서 주로 설명하는 차량 (hyundai_ioniq6, kia_ev6, 또는 both)"
    },
    ...
  ]
}
...
- titleMain은 비교 대상을 명시 (예: "아이오닉6 vs EV6!")
- titleSub는 호기심 유발 문구 (예: "찐 승자는?", "승자는 과연?")
- 각 segment의 car 필드는 해당 세그먼트에서 주로 설명하는 차량을 명시 (hyundai_ioniq6, kia_ev6, both 중 하나)
- 비교 영상의 경우 각 차량을 번갈아 설명하도록 구성
...
`;
    ...
    // TTS 텍스트에서 자막을 자동 생성 (TTS와 동일한 내용)
    const segments = scriptData.segments.map((segment: any) => ({
      text: segment.text,
      subtitles: splitTextToSubtitles(segment.text, 10),
